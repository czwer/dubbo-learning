...
（6）完整编译：
    ...
    方案1：编译并发布到本机.m2方便其它项目使用
        ./gradlew -x :spring-boot-project:spring-boot-docs:dokkatooGeneratePublicationHtml publishToMavenLocal --parallel
    方案2：并行编译（加快速度）
        ./gradlew -x :spring-boot-project:spring-boot-docs:dokkatooGeneratePublicationHtml publishToMavenLocal --parallel --parallel
